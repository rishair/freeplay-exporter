...
export class FreeplayExporter implements SpanExporter {
   ...
   */
  private formatTraceIdToUUID(traceId: string): string {
    if (traceId.length !== 32) {
      throw new Error("Invalid traceId length; expected 32 characters");
    }
    return `${traceId.slice(0, 8)}-${traceId.slice(8, 12)}-${traceId.slice(
      12,
      16
    )}-${traceId.slice(16, 20)}-${traceId.slice(20)}`;
  }
   ...
   */
  private extractInputs(attributes: Record<string, any>): Record<string, any> {
  ...
  export(
    ...
    for (const span of spans) {
      ...
      if (
        ["ai.generateText.doGenerate", "ai.streamText.doStream"].includes(
          span.name
        )
      ) {
        ...
        try {
          ...
          console.warn(
            `Skipping span ${span.name} due to invalid traceId: ${error.message}`
          );
          continue;
        }
        ...
        if (promptMessagesStr) {
          try {
            messages = JSON.parse(promptMessagesStr as string);
          } catch (e: unknown) {
            const error = e as Error;
            console.warn(
              `Failed to parse ai.prompt.messages for span ${span.name}: ${error.message}`
            );
            continue;
          }
        } else {
        ...
        const promise = fetch(url, {
          ...
          .catch((error: unknown) => {
            ...
            console.error(
              `Failed to export span ${span.name} to Freeplay: ${err.message}`
            );
          })
          .finally(() => {
